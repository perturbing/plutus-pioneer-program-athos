...
async function readStateScript(): Promise<L.SpendingValidator> {
  ...
  return {
    type: "PlutusV2",
    script: L.applyParamsToScript<Params>(script.cborHex,[policyIdFree],Params)
  }
}
...
async function mintFree(): Promise<L.TxHash> {
  const tkn: L.Unit = L.toUnit(policyIdFree,L.fromText("The State Token3"));
  const tx = await lucid
    .newTx()
    .mintAssets({ [tkn]: 1n}, L.Data.void())
    .payToContract(lockingAddress, L.Data.to(new L.Constr(0,["00"])),{[tkn]: 1n})
...
async function updateState() {
  ...
  if (ourUTxO && ourUTxO.length > 0) {
    const tx = await lucid
    .newTx()
    .collectFrom(ourUTxO, L.Data.to(new L.Constr(0,["00000000000000"])))
    .attachSpendingValidator(stateScript)
    //.payToContract(lockingAddress, L.Data.to(new L.Constr(0,["01"])),{[tkn]: 1n})

...
import { numberOfExamsStarted } from "@/utilities/utilities";
...
export default function Home() {
    ...
    const { lucid, wAddr, blockfrostKey } = appState;
    const [examsStartedUntilNow, setExamsStartedUntilNow] = useState(0);

    useEffect(() => {
        if (!lucid) return;
        numberOfExamsStarted(lucid).then((res) => {
            res && setExamsStartedUntilNow(res);
        });
    }, [lucid]);
